...
from gpt_promps import Gpt_promps
...
def output_to_json(output_path: str, expanded_slide_explanations: Dict[int, str]) -> None:
    ...
    """
    json_to_save = {
        "number of slides": len(expanded_slide_explanations.values()),
        "explained slides": expanded_slide_explanations}
...
def main():
    args = parse_args()
    presentation_parser = PresentationParser(args.pptx_file)
    presentation_parser.parse()
    slide_expander = GPTSlideExpander()

    presentation_main_topic = args.topic
